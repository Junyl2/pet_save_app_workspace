...
export default function TaxInvoiceDetails() {
  ...
  return (
    ...
    >
        ...
        <div className={styles.sectionInner}>
          ...
          {details.items.map((item, idx) => (
            <div key={idx} className={styles.paymentRow}>
              ...
              <div className={styles.infoValue}>
                {item.productName} ({item.quantity}개) — {KRW(item.totalAmount)}
              </div>
            </div>
          ))}
      ...
      <div className={styles.section}>
        <div className={styles.sectionInner}>
          <div className={styles.sectionTitle}>세금계산서</div>
          <div className={styles.infoRow}>
            <div className={styles.infoLabel}>발행번호</div>
            <div className={styles.infoValue}>{details.issueNumber ?? '-'}</div>
          </div>
          <div className={styles.infoRow}>
            <div className={styles.infoLabel}>상호명</div>
            <div className={styles.infoValue}>
              {details.companyInfo.businessName ?? '-'}
            </div>
          </div>
          <div className={styles.infoRow}>
            <div className={styles.infoLabel}>대표자명</div>
            <div className={styles.infoValue}>
              {details.companyInfo.representativeName ?? '-'}
            </div>
          </div>
          <div className={styles.infoRow}>
            <div className={styles.infoLabel}>사업자주소</div>
            <div className={styles.infoValue}>
              {details.companyInfo.businessAddress ?? '-'}
            </div>
          </div>
          <div className={styles.infoRow}>
            <div className={styles.infoLabel}>업태/종목</div>
            <div className={styles.infoValue}>
              {details.companyInfo.businessType ?? '-'}
              {details.companyInfo.businessCategory
                ? ` / ${details.companyInfo.businessCategory}`
                : ''}
            </div>
          </div>
          <div className={styles.infoRow}>
            ...
            <div className={styles.infoValue}>
              {details.companyInfo.businessEmail ?? '-'}
            </div>
          </div>
        </div>
      </div>
